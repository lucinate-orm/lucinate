...
import { getDefaultModelAdapter } from '../default_model_adapter.js';
import { Preloader } from '../preloader/index.js';
import { proxyHandler } from './proxy_handler.js';
import { INTERNAL_INSTANCE_PROPERTIES } from './internal_instance_properties.js';
 ...
 */
let globalNamingStrategy = new CamelCaseNamingStrategy();
...
let BaseModelImpl = (() => {
    ...
    let _classThis;
    var BaseModelImpl = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            BaseModelImpl = _classThis = _classDescriptor.value;
        ...
        static set namingStrategy(strategy) {
            const Model = this;
            if (Model === BaseModelImpl) {
         ...
         */
        static filter(input, FilterCtor) {
            this.boot();
         ...
         * - Returns: 'users_email'
         */
        static $getColumnAlias(columnName) {
            this.boot();
        ...
        static boot() {
            ...
            if (this.booted === true) {
                if (!this.$adapter) {
                    const fallbackEarly = getDefaultModelAdapter();
                    if (fallbackEarly) {
                        this.useAdapter(fallbackEarly);
                    }
                }
                return;
            }
            ...
            this.$defineProperty('$keys', {
                ...
                return {
                    ...
                    columnAliasesToAttributes: new ModelKeys(Object.assign({}, value.columnAliasesToAttributes.all())),
                };
            });
            /**
             * Define columns
        ...
        $consumeAdapterResult(adapterResult, sideloadedAttributes) {
            ...
            if (isObject(adapterResult)) {
                Object.keys(adapterResult).forEach((key) => {
                     ...
                     */
                    let attributeName = Model.$keys.columnAliasesToAttributes.get(key) || Model.$keys.columnsToAttributes.get(key);
                    if (attributeName) {
    ...
    return BaseModelImpl = _classThis;
})();
