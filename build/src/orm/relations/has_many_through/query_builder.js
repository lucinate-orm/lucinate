...
export class HasManyThroughQueryBuilder extends BaseQueryBuilder {
    ...
    getGroupLimitQuery() {
        ...
        this.select(this.client.raw(`row_number() over (PARTITION BY ?? ORDER BY ?? ${direction}) as ${rowName}`, [this.prefixThroughTable(this.relation.foreignKeyColumnName), column])).as('lucinate_temp');
