...
export class MigrationSource {
    ...
    async getDirectoryFiles(directoryPath) {
        ...
        const fromLocation = typeof root === 'string'
            ? pathToFileURL(root.endsWith('/') ? root : `${root}/`)
            : root;
        const { files } = await sourceFiles(fromLocation, directoryPath, this.config.migrations?.naturalSort || false);
        return files;
    }
    /**
    ...
    getMigrationsPaths() {
        ...
        const raw = directories && directories.length ? directories : [`./${defaultDirectory}`];
