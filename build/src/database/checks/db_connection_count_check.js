...
export class DbConnectionCountCheck extends BaseCheck {
     ...
     */
    #getConnectionsCountMetadata(active) {
        return {
            connectionsCount: {
                active,
     ...
     * ```
     */
    warnWhenExceeds(connectionsCount) {
     ...
     * ```
     */
    failWhenExceeds(connectionsCount) {
    ...
    async run() {
        try {
            ...
            if (!connectionsCount) {
                return Result.ok(`Check skipped. Unable to get active connections for ${this.#client.dialect.name} dialect`).mergeMetaData(this.#getConnectionMetadata());
            }
            /**
             * Check if we have crossed the failure threshold
             */
            if (connectionsCount > this.#failThreshold) {
                return Result.failed(`There are ${connectionsCount} active connections, which is above the threshold of ${this.#failThreshold} connections`)
            ...
            if (connectionsCount > this.#warnThreshold) {
                return Result.warning(`There are ${connectionsCount} active connections, which is above the threshold of ${this.#warnThreshold} connections`)
                    .mergeMetaData(this.#getConnectionMetadata())
                    .mergeMetaData(this.#getConnectionsCountMetadata(connectionsCount));
            }
            return Result.ok(`There are ${connectionsCount} active connections, which is under the defined thresholds`)
                .mergeMetaData(this.#getConnectionMetadata())
                .mergeMetaData(this.#getConnectionsCountMetadata(connectionsCount));
        }
        catch (error) {
            return Result.failed(error instanceof Error ? error.message : 'Connection failed', error).mergeMetaData(this.#getConnectionMetadata());
        }
    }
}
//# sourceMappingURL=db_connection_count_check.js.map