...
const { patchKnex } = require('knex-dynamic-connection') as typeof import('knex-dynamic-connection')
const { resolveClientNameWithAliases } = require('knex/lib/util/helpers.js') as {
  resolveClientNameWithAliases: (name: string) => string
}
...
export class Connection extends EventEmitter implements ConnectionContract {
  ...
  constructor(
    ...
    super()
    this.validateConfig()
    this.clientName = resolveClientNameWithAliases(this.config.client) as ConnectionContract['clientName']
